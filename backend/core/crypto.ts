...
export async function decryptJson<T>(value: string, secret: string): Promise<T> {
  ...
  if (!encodedIv || !encodedCiphertext) throw new Error("Invalid encrypted value");
  const plaintext = await crypto.subtle.decrypt({ name: "AES-GCM", iv: unb64(encodedIv) }, await encryptionKey(secret), unb64(encodedCiphertext));
