...
export class AppError extends Error {
  ...
  constructor(code: ProblemCode, detail: string, param?: string, cause?: unknown) {
    super(detail);
    this.name = "AppError";
    this.code = code;
    this.param = param;
...
export function toProblem(
  ...
  const appError = error instanceof AppError
    ? error
    : new AppError("UPSTREAM_UNAVAILABLE", "The mail provider is unavailable.", undefined, error);
...
export function jsonProblem(error: unknown, request: Request, baseUrl: string): Response {
  const request_id = request.headers.get("x-request-id") || requestId();
  const problem = toProblem(error, new URL(request.url).pathname, request_id, baseUrl);
