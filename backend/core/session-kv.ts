...
export class EncryptedSessionStore implements SessionStore {
  constructor(private readonly storage: KeyValueStore, private readonly config: BackendConfig) {}

  async get(sid: string): Promise<SessionRecord | null> {
    const value = await this.storage.get(`session:${sid}`);
    if (!value) return null;
    try {
      const record = await decryptJson<SessionRecord>(value, this.config.jwtSecret);
  ...
  async put(record: SessionRecord): Promise<void> {
    ...
    await this.storage.put(`session:${record.sid}`, await encryptJson(record, this.config.jwtSecret), ttl);
  }
  ...
  async revokeFamily(family: string): Promise<void> {
    await this.storage.put(`revoked-family:${family}`, "1", this.config.refreshTtlSeconds);
  }
